...
let render = function (component, initState = {}, mountNode = "app") {
  initState.render = function (stateRepresentation, options = {}) {
    const start = options.focus
      ? document.getElementById(options.focus).selectionStart
      : 0;
  ...
  initState.render(
    typeof stateRepresentation === "function"
      ? stateRepresentation()
      : stateRepresentation
  );
};
...
let Editor = function ({ defaultValue, render }) {
  let state = { value: defaultValue || "Type some text here!", render };
...
// Display Components
render(HelloMessage, { name: "Jane" }, "hello");

render(Timer, { seconds: 0 }, "timer");

render(TodoApp, {}, "todo");

render(Editor, {}, "markdown");
